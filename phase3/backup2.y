...
function: functionIdent SEMICOLON beginparamIdent declarations endparamIdent BEGINLOCALS declarations ENDLOCALS BEGINBODY statements ENDBODY
        {
            variable myVar = {"endfunc", "endfunction"};
            insArr(vArr, myVar);
            paramCnt = 0;
        }
        ;
...
void genMil(variable* arr)
{
    int i;
    for(i = 0; i < 1024; i++)
    {
        if(strcmp(arr[i].vType, "function") == 0)
        {
            printf("func %s\n", arr[i].vName);
        }
        else if(strcmp(arr[i].vType, "endfunction") == 0)
        {
            printf("%s\n", arr[i].vName);
        }
        else if(strcmp(arr[i].vType, "intParam") == 0 )
        {
            printf(". %s\n", arr[i].vName);
            printf("= %s, $%d\n", arr[i].vName, arr[i].vID);
        }
        else if(strcmp(arr[i].vType, "intDec") == 0)
        {
            printf(". %s\n", arr[i].vName);
        }
        else if(strcmp(arr[i].vType, "arrDec") == 0)
        {
            printf(".[] %s, %d\n", arr[i].vName, arr[i].vID);
        }
        else if(strcmp(arr[i].vType, "temp") == 0)
        {
            printf(". %s%d\n", arr[i].vName, arr[i].vID);
        }
    }
